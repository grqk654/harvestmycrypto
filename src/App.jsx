...
function ToolCTA({ text="Use the Free Calculator →", navigate }) {
  return (
    <div style={{ background:"linear-gradient(135deg,#1a2e1a,#0f1f0f)", borderRadius:"12px",
      ...
      <button onClick={()=>navigate("tools/tlh")} style={{ padding:"0.6rem 1.3rem",
    ...
    <div className="fade-in">
      ...
      <div className="fade-up" style={{ animationDelay:"300ms", marginBottom:"2rem" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:"0.9rem" }}>
          {[
            {id:"tools/tlh",     icon:"◈", name:"Tax-Loss Harvesting", desc:"See your exact savings from selling at a loss."},
            {id:"tools/gains",   icon:"◆", name:"Capital Gains",       desc:"Your tax bill before you sell."},
            {id:"tools/swap",    icon:"⇌", name:"Swap Tax Impact",     desc:"Hidden tax cost of coin-to-coin trades."},
            {id:"tools/carry",   icon:"↻", name:"Carryforward Tracker",desc:"How losses roll into future years."},
            {id:"tools/whatif",  icon:"★", name:"What If I Invested",  desc:"What $1,000 in Bitcoin years ago is worth today."},
            {id:"tools/income",  icon:"⬡", name:"Crypto Income Tax",   desc:"Tax on staking, mining & airdrops."},
            {id:"tools/bracket", icon:"⚖", name:"Tax Bracket Impact",  desc:"See which bracket your crypto gains hit."},
          ].map(t=>(
...
function ToolsPage({ navigate, activeTool="tlh" }) {
  const tools = [
  ...
  return (
    <div className="fade-in">
      ...
      <div style={{ display:"flex", gap:"0", overflowX:"auto", scrollbarWidth:"none",
        ...
        {tools.map(t=>(
          <button key={t.id} onClick={()=>navigate(`tools/${t.id}`)} style={{
            flexShrink:0, padding:"0.6rem 0.7rem", background:activeTool===t.id?"#1a3a1a":"transparent",
            border:"none", borderRadius:"7px", color:activeTool===t.id?"#6ee85a":"#3a5a3a",
      ...
      {activeTool==="tlh"     && <TLHCalc />}
      {activeTool==="gains"   && <GainsCalc />}
      {activeTool==="swap"    && <SwapCalc />}
      {activeTool==="carry"   && <CarryCalc />}
      {activeTool==="whatif"  && <WhatIfCalc />}
      {activeTool==="income"  && <IncomeCalc />}
      {activeTool==="bracket" && <BracketCalc />}
      <AffiliateSection />
...
export default function App() {
  ...
  const subPage = page.split('/')[1] || 'tlh';
  const NAV_ITEMS = [
    {id:"home",   label:"Home"},
  ...
  return (
    <div ref={containerRef} style={{ height:"100vh", overflowY:"auto",
          ...
          <div className="hide-mobile" style={{ display:"flex", alignItems:"center", gap:"0.2rem" }}>
            {NAV_ITEMS.map(n=>(
              <button key={n.id} onClick={()=>navigate(n.id)} style={{
                ...
                borderBottom: topPage===n.id?"2px solid #6ee85a":"2px solid transparent",
                color: topPage===n.id?"#6ee85a":"#2e4e2e",
                cursor:"pointer", fontFamily:"'JetBrains Mono',monospace",
                fontSize:"0.65rem", letterSpacing:"0.1em", textTransform:"uppercase",
                transition:"all 0.2s" }}
                onMouseEnter={e=>{if(topPage!==n.id)e.currentTarget.style.color="#9aca9a";}}
                onMouseLeave={e=>{if(topPage!==n.id)e.currentTarget.style.color="#2e4e2e";}}>
                {n.label}
              </button>
            ))}
            <button onClick={()=>navigate("tools/tlh")} style={{ marginLeft:"0.5rem",
      ...
      <main style={{ position:"relative", zIndex:1, maxWidth:"960px",
        ...
        {topPage==="home"   && <HomePage navigate={navigate} />}
        {topPage==="tools"  && <ToolsPage navigate={navigate} activeTool={subPage} />}
        {topPage==="guides" && <GuidesPage navigate={navigate} />}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",
            ...
            <div>
              ...
              {["Tax-Loss Harvesting","Capital Gains","Swap Impact","Carryforward","What If","Crypto Income","Tax Bracket"].map((l,i)=>(
                <button key={i} onClick={()=>navigate(["tools/tlh","tools/gains","tools/swap","tools/carry","tools/whatif","tools/income","tools/bracket"][i])} style={{
